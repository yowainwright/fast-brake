...
try {
  await $\`bunx oxlint src tests --fix --ignore-pattern='tests/fixtures/*'\`;
  await $\`bunx prettier --write "**/*.{ts,tsx,js,jsx,json,md}"\`;
  await $\`bun test --preload ./tests/setup.ts tests/unit tests/integration/integration.test.ts --timeout 5000\`;
