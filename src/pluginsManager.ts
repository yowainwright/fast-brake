import { Plugin, PluginConfig, PluginResult, PluginContext } from './plugins/types';
...
export class PluginManager {
  ...
  private patternToPlugin: Map<string, { plugin: Plugin; pattern: any }> = new Map();
        ...
        if (this.pluginRegistry.has(name)) {
          ...
          plugin = {
            ...
            validate: basePlugin.validate ? 
              (ctx, matches) => basePlugin.validate!({ ...ctx, options: { ...ctx.options, ...options } }, matches) :
              undefined
          };
        }
  ...
  detect(code: string, options?: Record<string, any>): PluginResult[] {
    ...
    while ((match = this.combinedPattern.exec(code)) !== null) {
      ...
      if (matchedPlugin && matchedPattern) {
        ...
        const result: PluginResult = {
          ...
          line: lineNum,
          column: column,
          snippet: code.substring(match.index, Math.min(match.index + 50, code.length)).replace(/\n/g, ' ').trim(),
    ...
    for (const [plugin, matches] of matchesByPlugin) {
      const validated = plugin.validate ? plugin.validate(context, matches) : matches;
