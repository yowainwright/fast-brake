...
const parsers = [
  ...
  },
  {
    name: "espree",
    fn: (code: string) =>
      espree.parse(code, { ecmaVersion: "latest", sourceType: "module" }),
    accuracy: "parsed",
  },
  {
    name: "meriyah",
...
export async function updateRootReadme(benchmarkTable: string): Promise<void> {
  ...
  if (startIndex === -1 || endIndex === -1) {
    ...
    console.log(
      "   Add <!-- BENCHMARK_START --> and <!-- BENCHMARK_END --> markers",
    );
    return;
  }
...
export async function updateSiteDocs(
  benchmarkData: BenchmarkSet[],
): Promise<void> {
...
export async function saveBenchmarkJson(
  benchmarkData: BenchmarkSet[],
): Promise<void> {
...
export async function updateSiteBenchmarkTable(
  benchmarkData: BenchmarkSet[],
): Promise<void> {
  ...
  try {
    ...
    const es2015Data =
      benchmarkData.find((set) => set.name.includes("ES2015")) ||
      benchmarkData[0];
    ...
    if (dataStart === -1 || dataEnd === 1) {
      console.warn(
        "⚠️  Could not find benchmarkData array in BenchmarkTable.tsx",
      );
      return;
    }
