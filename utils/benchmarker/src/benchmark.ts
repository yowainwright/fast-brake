...
async function runBenchmark(
  ...
  for (let i = 0; i < 10; i++) {
    try {
      await parser.parse(testCase.code);
    } catch {}
  }
  ...
  let accuracy = "";
  if (parser.validate) {
    const result = await parser.validate(testCase.code);
...
async function main() {
  ...
  console.log(
    pc.gray("Comparing fast-brake against popular JavaScript parsers\n"),
  );
  ...
  for (const testCase of testCases) {
    ...
    for (const [name, parser] of Object.entries(parsers)) {
      ...
      try {
        const result = await runBenchmark(name, parser, testCase);
        results.push(result);
    ...
    const baseline =
      results.find((r) => r.parser === "fast-brake")?.timeMs || 1;
    results.forEach((r) => {
  ...
  const fastBrakeAvg =
    Array.from(allResults.values())
      .map(
        (results) =>
          results.find((r) => r.parser === "fast-brake")?.opsPerSec || 0,
      )
  ...
  const babelAvg =
    Array.from(allResults.values())
      .map(
        (results) =>
          results.find((r) => r.parser === "@babel/parser")?.opsPerSec || 0,
      )
      .reduce((a, b) => a + b, 0) / allResults.size;

  console.log(
    pc.green(`✓ fast-brake: ${fastBrakeAvg.toFixed(0)} ops/sec average`),
  );
  console.log(
    pc.yellow(`✓ @babel/parser: ${babelAvg.toFixed(0)} ops/sec average`),
  );
  console.log(
    pc.bold(
      `✓ Speed advantage: ${(fastBrakeAvg / babelAvg).toFixed(1)}x faster\n`,
    ),
  );
  ...
  console.log(
    pc.gray("  • Targeted feature detection vs complete syntax analysis"),
  );
