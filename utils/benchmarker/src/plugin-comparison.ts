...
} from "fast-brake/plugins/telemetry";
...
const pluginConfigs: PluginBenchmark[] = [
  ...
  },
  {
    ...
    getPlugin: () =>
      createBrowserlistPlugin(["chrome 60", "firefox 55", "safari 11"]),
...
async function main() {
  ...
  console.log(
    pc.gray(`Test code size: ${Buffer.byteLength(code, "utf-8")} bytes\n`),
  );
  ...
  for (const config of pluginConfigs) {
    process.stdout.write(
      pc.gray(`  ${config.name.padEnd(20)} - ${config.description.padEnd(40)}`),
    );
  ...
  if (singleES5 && allES) {
    const overhead = (
      ((allES.timeMs - singleES5.timeMs) / singleES5.timeMs) *
      100
    ).toFixed(1);
    console.log(
      pc.yellow(`• All ES versions vs Single ES5: ${overhead}% overhead`),
    );
    console.log(
      pc.gray(`  Single ES5: ${singleES5.opsPerSec.toFixed(0)} ops/sec`),
    );
  ...
  if (allES && defaultBrowsers) {
    const overhead = (
      ((defaultBrowsers.timeMs - allES.timeMs) / allES.timeMs) *
      100
    ).toFixed(1);
    ...
    console.log(
      pc.gray(
        `  Default Browsers: ${defaultBrowsers.opsPerSec.toFixed(0)} ops/sec`,
      ),
    );
  }
  ...
  if (singleES5 && strictTelemetry) {
    const totalOverhead = (
      ((strictTelemetry.timeMs - singleES5.timeMs) / singleES5.timeMs) *
      100
    ).toFixed(1);
    console.log(
    ...
    console.log(
      pc.gray(
        `  Strict Telemetry: ${strictTelemetry.opsPerSec.toFixed(0)} ops/sec`,
      ),
    );
  }
    ...
    if (r) {
      comparisonTable.push([
        ...
        r.opsPerSec > typicalSpeeds.Meriyah
          ? pc.green(
              `${(r.opsPerSec / typicalSpeeds.Meriyah).toFixed(1)}x faster`,
            )
          : pc.yellow(`${(r.opsPerSec / typicalSpeeds.Meriyah).toFixed(1)}x`),
        r.opsPerSec > typicalSpeeds.Cherow
          ? pc.green(
              `${(r.opsPerSec / typicalSpeeds.Cherow).toFixed(1)}x faster`,
            )
  ...
  console.log(
    pc.green("• Use single ES version plugins when you know your target"),
  );
  console.log(
    pc.green('• "All ES" plugin adds ~20-30% overhead but covers everything'),
  );
  console.log(pc.yellow("• Browser checks add another ~10-20% overhead"));
  console.log(
    pc.yellow(
      "• Even with all plugins, still significantly faster than AST parsers",
    ),
  );
  ...
  console.log(
    pc.gray("  - Production builds: All + Browser (complete coverage)"),
  );
