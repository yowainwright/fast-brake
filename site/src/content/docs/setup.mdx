...
module.exports = async function(source) {
  ...
  const isCompatible = await check(source, { 
    target, 
    quick: mode === 'quick' 
  });
  
  if (!isCompatible) {
    const features = await detect(source, { target, quick: mode === 'quick' });
...
// Detect features with fastBrake
const results = await fastBrake(code);
