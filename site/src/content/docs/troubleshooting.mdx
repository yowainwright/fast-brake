...
fast-brake is a blazing-fast ES feature detection library that analyzes JavaScript code to identify ECMAScript features and verify compatibility with target ES versions. It uses a revolutionary two-phase detection system that's 7x faster than traditional AST parsers.
...
2. **Phase 2**: Intelligent tokenizer validation to eliminate false positives
...
- ✅ **7x faster** than full mode (~0.013ms per file)
- ✅ Perfect for **development builds** and **hot reloading**
- ⚠️ **95% accuracy** - may have false positives from strings/comments
...
fast-brake supports **ES5 through ES2022** with comprehensive coverage:
...
| Tool | 1000 Files | Accuracy | Memory |
|------|------------|----------|---------|
| **fast-brake (Quick)** | **13ms** | 95% | **4MB** |
| **fast-brake (Full)** | **55ms** | **99.9%** | **4MB** |
| Babel Parser | ~100ms | 99.9% | ~15MB |
| ESLint Parser | ~120ms | 99.9% | ~18MB |
| Acorn Parser | ~80ms | 99.9% | ~12MB |
