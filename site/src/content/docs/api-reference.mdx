...
if (check(userCode, { target: 'es2018' })) {
  ...
  console.log(' Code requires transpilation for ES2018');
}
```
...
**Maximum performance** with targeted detection using plugins.

```javascript
import { detectWithPlugins, checkWithPlugins } from 'fast-brake';

// Targeted detection - up to 85x faster!
const features = detectWithPlugins(code, ['es2015']);
const isCompatible = checkWithPlugins(code, ['es2015']);

// Comprehensive detection
const allFeatures = detectWithPlugins(code, ['all']);

// Perfect for:
// - Development builds with known targets
// - Hot reloading
// - CI/CD with specific ES version targets
// - Maximum performance requirements
```

**Characteristics:**
-  **Ultra-fast**: Up to 85x faster with single plugins
-  **Targeted detection**: Load only what you need
-  **Flexible**: Combine multiple plugins
-  **Use case**: Development, hot reloading, CI/CD
        ...
        chunks.forEach(chunk => {
          chunk.files.forEach(filename => {
            ...
            if (!check(source, { target: this.target })) {
              ...
              console.warn(` ${filename} contains incompatible features:`, 
