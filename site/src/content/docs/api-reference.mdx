...
- `detect(code: string): DetectedFeature[]` - Detect patterns synchronously
- `check(code: string, options: DetectionOptions): boolean` - Check compatibility synchronously

### `fastBrake(code)` or `fastBrake(options)`

Can be used in two ways:
1. **Direct detection:** Pass code string for quick pattern detection
2. **Factory pattern:** Pass options to create an async instance with custom plugins
...
**Returns:** 
- With code: `Promise<DetectedFeature[]>` - Array with detected patterns
- With options: `Promise<FastBrakeAPI>` with async `detect()` and `check()` methods
...
if (await check(userCode, { target: 'es2018' })) {
  console.log('✓ Code is compatible with es2018 rules');
} else {
  console.log('⚠ Code contains patterns beyond es2018');
}
```
        ...
        for (const chunk of chunks) {
          for (const filename of chunk.files) {
            ...
            if (!isCompatible) {
              ...
              console.warn(`⚠ ${filename} contains pattern:`, patterns[0]?.name);
            }
          }
        }
        callback();
  ...
  for (const file of files) {
    try {
      ...
      if (isCompatible) {
        ...
        console.error(`❌ ${file}: Contains ${patterns[0]?.name} (rule: ${patterns[0]?.version})`);
        hasErrors = true;
      }
    } catch (error) {
