...
for (const file of files) {
  ...
  for (let i = 0; i < iterations; i++) {
    detect(file.content, { target: 'es2015', quick: true });
  }
  ...
  for (let i = 0; i < iterations; i++) {
    detect(file.content, { target: 'es2015', quick: false });
  }
...
console.log(`  Quick mode: ${batchQuickTime}ms total (${(batchQuickTime / batchSize).toFixed(3)}ms per file)`);
console.log(`  Full mode:  ${batchFullTime}ms total (${(batchFullTime / batchSize).toFixed(3)}ms per file)`);
...
if (batchQuickTime < requirement) {
  console.log(`  ✓ 1000 files in ${batchQuickTime}ms < ${requirement}ms requirement`);
} else {
  console.log(`  ✗ 1000 files in ${batchQuickTime}ms > ${requirement}ms requirement`);
}
...
if (avgQuickTime < requirementPerFile) {
  console.log(`  ✓ Single file in ${avgQuickTime.toFixed(3)}ms < ${requirementPerFile}ms requirement`);
} else {
  console.log(`  ✗ Single file in ${avgQuickTime.toFixed(3)}ms > ${requirementPerFile}ms requirement`);
}
...
for (const file of files) {
  ...
  console.log(`  ${file.name}: ${features.length} features detected, minimum version: ${minVersion}`);
}